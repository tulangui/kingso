 ...
 * 第一个整数和 索引单元的距离总是 1
 */
static const int GROUP_VARINT_IDX_ARR[256][3] =
{
    /* 00 00 00 00 */ { 2, 3, 4 },
    /* 00 00 00 01 */ { 2, 3, 4 },
    /* 00 00 00 10 */ { 2, 3, 4 },
...
/**
 * group varint 的索引表，
 * 表示的是 下一个索引单元 和 第一个索引单元的距离(byte)
 */
static const int GROUP_VARINT_NEXT_IDX[256] =
{
    5, 6, 7, 8, 6, 7, 8, 9,
    7, 8, 9, 10, 8, 9, 10, 11,
    6, 7, 8, 9, 7, 8, 9, 10,
...
/**
 * 对整数数组进行 group varint编码, 一次处理2个uint64整数
 *
 ...
 */
inline uint8_t *
group_varint_encode_uint64 ( uint64_t * valueArr, uint8_t * target)
{
    ...
    GRPVARINT64 tmp1,tmp2;
    tmp1.v64 = valueArr[0];
    tmp2.v64 = valueArr[1];

    uint8_t * buf = target + 1;

    if ( tmp1.v32[0] < MAX_UINT8 )
    {
        len1   = 1;
        buf[0] = (uint8_t) tmp1.v32[0];
    }
    else if ( tmp1.v32[0] < MAX_UINT16 )
    {
        len1 = 2;
        ((uint16_t *)(buf))[0] = (uint16_t) tmp1.v32[0];
    }
    else if ( tmp1.v32[0] < MAX_UINT24 )
    {
        len1 = 3;
        ((uint32_t *)(buf))[0] = ( tmp1.v32[0] );
    } else  {
        len1 = 4;
        ((uint32_t *)(buf))[0] = tmp1.v32[0];
    }
    ...
    /*  处理第二个数字   */
    if ( tmp1.v32[1] < MAX_UINT8 )
    {
        len2 = 1;
        buf[len] = (uint8_t) tmp1.v32[1];
    }
    else if ( tmp1.v32[1] < MAX_UINT16 )
    {
        len2 = 2;
        ((uint16_t *)(buf + len))[0] = (uint16_t) tmp1.v32[1];
    }
    else if ( tmp1.v32[1] < MAX_UINT24 )
    {
        len2 = 3;
        ((uint32_t *)(buf + len))[0] = ( tmp1.v32[1] );
    } else  {
        len2 = 4;
        ((uint32_t *)(buf + len))[0] = tmp1.v32[1];
    }
    ...
    /*  处理第3个数字   */
    if ( tmp2.v32[0] < MAX_UINT8 )
    {
        len3 = 1;
        buf[len] = (uint8_t) tmp2.v32[0];
    }
    else if ( tmp2.v32[0] < MAX_UINT16 )
    {
        len3 = 2;
        ((uint16_t *)(buf + len))[0] = (uint16_t) tmp2.v32[0];
    }
    else if ( tmp2.v32[0] < MAX_UINT24 )
    {
        len3 = 3;
        ((uint32_t *)(buf + len))[0] = ( tmp2.v32[0] );
    } else  {
        len3 = 4;
        ((uint32_t *)(buf + len))[0] = tmp2.v32[0];
    }
    ...
    /*  处理第4个数字   */
    if ( tmp2.v32[1] < MAX_UINT8 )
    {
        len4 = 1;
        buf[len] = (uint8_t) tmp2.v32[1];
    }
    else if ( tmp2.v32[1] < MAX_UINT16 )
    {
        len4 = 2;
        ((uint16_t *)(buf + len))[0] = (uint16_t) tmp2.v32[1];
    }
    else if ( tmp2.v32[1] < MAX_UINT24 )
    {
        len4 = 3;
        ((uint32_t *)(buf + len))[0] = ( tmp2.v32[1] );
    } else  {
        len4 = 4;
        ((uint32_t *)(buf + len))[0] = tmp2.v32[1];
    }
 ...
 * 下面的方式进行解码， 性能搞， 但是  因为函数体大， 可能会导致 inline失败
 *
 * @param buf         输入的buf
 ...
 */
inline const uint8_t *
group_varint_decode_uint32_2 ( const uint8_t * buf, uint32_t * valueArr)
{
    ...
    return buf + GROUP_VARINT_NEXT_IDX[ idx ];
}
...
group_varint_decode_uint64 ( const uint8_t * buf, uint64_t * valueArr)
{
    ...
    register uint8_t   idx   = buf[0];
    register int       len1  = GROUP_VARINT_IDX_ARR[idx][0] - 1;
    const    uint8_t * star2 = star1 + len1;
    register int       len2  = GROUP_VARINT_IDX_ARR[idx][1] - GROUP_VARINT_IDX_ARR[idx][0];
    const    uint8_t * star3 = star2 + len2;
    register int       len3  = GROUP_VARINT_IDX_ARR[idx][2] - GROUP_VARINT_IDX_ARR[idx][1];
    const    uint8_t * star4 = star3 + len3;
    register int       len4  = GROUP_VARINT_NEXT_IDX[idx] - GROUP_VARINT_IDX_ARR[idx][2];

    switch ( len1 )
    {
        case 1 : tmp1.v32[ 0 ] = *(uint8_t *)  star1;   break;
        case 2 : tmp1.v32[ 0 ] = *(uint16_t *) star1;   break;
        case 3 : tmp1.v32[ 0 ] = (*(UINT24_T *)star1).value; break;
        case 4 : tmp1.v32[ 0 ] = *(uint32_t *) star1;   break;
    }

    switch ( len2 )
    {
        case 1 : tmp1.v32[ 1 ] = *(uint8_t *)  star2;   break;
        case 2 : tmp1.v32[ 1 ] = *(uint16_t *) star2;   break;
        case 3 : tmp1.v32[ 1 ] = (*(UINT24_T *)star2).value; break;
        case 4 : tmp1.v32[ 1 ] = *(uint32_t *) star2;   break;
    }

    switch ( len3 )
    {
        case 1 : tmp2.v32[ 0 ] = *(uint8_t *)  star3;   break;
        case 2 : tmp2.v32[ 0 ] = *(uint16_t *) star3;   break;
        case 3 : tmp2.v32[ 0 ] = (*(UINT24_T *)star3).value; break;
        case 4 : tmp2.v32[ 0 ] = *(uint32_t *) star3;   break;
    }

    switch ( len4 )
    {
        case 1 : tmp2.v32[ 1 ] = *(uint8_t *)  star4;   break;
        case 2 : tmp2.v32[ 1 ] = *(uint16_t *) star4;   break;
        case 3 : tmp2.v32[ 1 ] = (*(UINT24_T *)star4).value; break;
        case 4 : tmp2.v32[ 1 ] = *(uint32_t *) star4;   break;
    }
...
group_varint_decode_uint32 ( const uint8_t * buf, uint32_t * valueArr)
{
    ...
    switch ( len1 )
    {
        ...
        case 4 : valueArr[ 0 ] = *(uint32_t *) star1;   break;
    }
    ...
    switch ( len2 )
    {
        ...
        case 4 : valueArr[ 1 ] = *(uint32_t *) star2;   break;
    }
    ...
    switch ( len3 )
    {
        ...
        case 4 : valueArr[ 2 ] = *(uint32_t *) star3;   break;
    }
    ...
    switch ( len4 )
    {
        ...
        case 4 : valueArr[ 3 ] = *(uint32_t *) star4;   break;
    }
