...
#include <iostream>
    ...
    class ResultFormatXMLConfig {
        ...
        inline long Utf8Width(const char *szStr, ESupportCodingForSearcherFormat eEncoding) {
            if (eEncoding == sc_GBK) {
                if (szStr[0] < 0 && szStr[1])
                    return 2;
                return 1;
            }
            else if (eEncoding == sc_UTF8) {
                if ((szStr[0] & 0x80) == 0)
                    return 1;
        ...
        char *ResultFormatXMLRef::convertRestrictedChar(const char *pchStr, ESupportCodingForSearcherFormat eEncodingType)
        {
            ...
            if((!_conf || _conf->getOutputData()) &&
                ...
                for(i = 0; i < pClusterResult->_nDocsReturn; i++) {
                    ...
                    for(int j = 0; j < fieldCount; j++) {
                        pTmp = pDocument->getField(j)->getName();
                        char *pEncodeStr = convertRestrictedChar(pDocument->getField(j)->getValue(), sc_UTF8);
