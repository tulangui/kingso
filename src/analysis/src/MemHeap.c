...
SHeap *suNewHeap( size_t iDefPageSize )
{
	...
	if (pPage->pMem == NULL) {
		fprintf(stderr,"suNewHeap> malloc() failed for page memory(size=%d)!\n", pPage->iPageSize);
		return NULL;
	}
...
void *suHeapAlloc(SHeap *pHeap, size_t iSize, size_t iAlign)
{
	...
	if (iSize > pCur->iPageSize || (iSize > pHeap->iDefPageSize && pCur->iBytesUsed + iSize + iOffset > pCur->iPageSize)) {
		...
		if (pNew->pMem == NULL) {
			fprintf( stderr, "suHeapAlloc> malloc() failed for large page memory(%d)!\n", iSize);
			return NULL;
		}
		...
		if (pPrev && pPrev->iBytesUsed + iSize + (iOffset = (iAlign - pPrev->iBytesUsed % iAlign) % iAlign) <= pPrev->iPageSize) {
			...
			if (pCur->pNext == NULL) {
				...
				if (pNew->pMem == NULL) {
					fprintf(stderr, "suHeapAlloc> malloc failed for new page memory(%d)!\n", pNew->iPageSize);
					return NULL;
				}
				pCur->pNext = pNew;
