...
namespace clustermap {
    ...
    const int MAX_CLUSTERNAME_LEN = 64;
    const int MAX_NODESPEC_LEN = 64;
    const int MAX_CLUSTER_COUNT = 200;
    const int MAX_NODE_COUNT = 100;

    struct NodeinfoWallResult_Cluster
    {     
        char Clustername[MAX_CLUSTERNAME_LEN];
        char NodeSpec[MAX_NODE_COUNT][MAX_NODESPEC_LEN];
        int nodecount;
        char ClusterType[32];
    };

    struct NodeinfoWallResult
    {
        NodeinfoWallResult_Cluster clusters[MAX_CLUSTER_COUNT];
        int clustercount;
    };
    ...
    {
        public:
            bool hasClusterName(const char *clustername, char *state = NULL);
            bool hasNodeSpec(const char *nodespec, char *state = NULL);
            bool hasNodeSpec(const char *ip, const uint16_t port, char *clustername, char *state);
            bool NodeinfoWall();
        private:
            bool isClusterStr(const char *str);
            bool isNodeStr(const char *str);
            NodeinfoWallResult result;
    };
